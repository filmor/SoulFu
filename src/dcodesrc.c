        ...
        if(data[2] == 'F' && data[3] == 'I' && data[4] == 'L' && data[5] == 'E' && data[6] == ':')
        {
            ...
            else
            {
                ...
                if(sdf_fix_filename(data+2+5, newfilename, &newfiletype))
                {
                    ...
                    {
                        log_message("ERROR:  File %s couldn't be found", data+2+5);
                        found_error = TRUE;
                    }
                }
                else
...
signed char src_compilerize(unsigned char* index, unsigned char* filename)
{
    ...
    else
    {
        ...
        log_message("ERROR:  Header file for %s.SRC wasn't generated, can't compile");
        return FALSE;
    }
    ...
    while(sdf_read_line())
    {
        ...
        if(sdf_read_file[0] != '#')
        {
            ...
            if(token_count > 0)
            {
                ...
                {
                    log_message("ERROR:  Line %d, Conditional without a nest", sdf_read_line_number);
                    found_error = TRUE;
                }
                ...
                if(indent == 0)
                {
                    ...
                    {
                        log_message("ERROR:  Line %d, Problem with function %s (corrupt header?)", sdf_read_line_number, token_buffer[i]);
                        found_error = TRUE;
                    }
                }
                ...
                if(operation_line)
                {
                    ...
                    if(indent > last_indent && last_indent != 0)
                    {
                        ...
                        {
                            // Throw an error
                            log_message("ERROR:  Line %d, Must have a conditional statement to indent", sdf_read_line_number);
                            found_error = TRUE;
                        }
                    }
                    ...
                    if(found_error == FALSE)
                    {
                        if(strcmp(token_buffer[0], "IF") == 0)
                        {
                            ...
                            if(src_buffer_used >= 5)
                            {
                                ...
                                {
                                    log_message("ERROR:  Line %d, Praxis syllabication error", sdf_read_line_number);
                                    found_error = TRUE;
                                }
                            }
...
signed char src_headerize(unsigned char* index, unsigned char* filename)
{
    ...
    while(sdf_read_line())
    {
        ...
        if(sdf_read_file[0] != '#' && indent == 0)
        {
            ...
            if(token_count > 0)
            {
                ...
                {
                    log_message("ERROR:  Line %d, function symbol '%s' not followed by ()", sdf_read_line_number, token_buffer[i]);
                    found_error = TRUE;
                }
            }
        }
    ...
    {
        log_message("ERROR:  No functions found");
        found_error = TRUE;
    }
    ...
    if(found_error == FALSE)
    {
        ...
        {
            log_message("ERROR:  Not enough memory to create header");
        }
    }
...
signed char src_stage_compile(unsigned char stage, unsigned char mask)
{
    ...
    repeat(i, sdf_num_files)
    {
        ...
        if(sdf_index[(i<<4)+4] & mask)
        {
            ...
            {
                #ifdef DEVTOOL
                    sofarsogood = src_headerize(sdf_index+(i<<4), filename) & sofarsogood;
                #else
            ...
            {
                #ifdef DEVTOOL
                    sofarsogood = src_compilerize(sdf_index+(i<<4), filename) & sofarsogood;
                #else
            ...
            {
                sofarsogood = src_functionize(sdf_index+(i<<4), filename) & sofarsogood;
            }
        }
    }
