                ...
                switch(i)
                {
                    ...
                    case SYS_FILEFTPFLAG:
                        if(j < sdf_num_files)
                        {
                            if(m)
                            {
                                sdf_index[(j<<4)+4] |= SDF_FLAG_NO_UPDATE;
                            }
                            else
                            {
                                sdf_index[(j<<4)+4] &= (~SDF_FLAG_NO_UPDATE);
                ...
                switch(i)
                {
                    ...
                    case SYS_FILESIZE:
                        if(j < sdf_num_files)
                        {
                            ...
                            if(opcode != SDF_FILE_IS_UNUSED)
                            {
                                k = sdf_read_unsigned_int(sdf_index+(j<<4)+4)&0x00FFFFFF;
...
//                    }
            ...
            case OPCODE_FILEOPEN:
                ...
                if(i == FILE_EXPORT)
                {
                    sdf_export_file(call_address);
                }
                if(i == FILE_IMPORT)
                {
                    sdf_add_file(call_address);
                }
                if(i == FILE_DELETE)
                {
